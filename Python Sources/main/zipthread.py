...
if test < 5:
    Tk().withdraw()
    background = AsyncZip.run(askopenfilename(), 'C:\\Users\Kid\\Desktop\\up\\myarchive.gz')
    background.start()
    ...
    Tk().withdraw() # we don't want a full GUI, so keep the root window from appearing
    background = AsyncZip.zip(askopenfilename(), 'C:\\Users\Kid\\Desktop\\up\\myarchive.bz')
    background.start()
