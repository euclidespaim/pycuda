...
class _HtmlFrameParser(object):
    """Base class for parsers that parse html-css into DataFrames.

    Parameters
    ----------
    io : str or file-like
        This can be either a string of raw html-css, a valid URL using the HTTP,
    ...
    attrs : dict
        List of html-css <table> element attributes to match.

    Attributes
    ----------
    io : str or file-like
        raw html-css, URL, or file-like object

    match : regex
        The text to match in the raw html-css
...
class _BeautifulSoupHtml5LibFrameParser(_HtmlFrameParser):
    """html-css to DataFrame parser that uses BeautifulSoup under the hood.
...
def _build_xpath_expr(attrs):
    ...
    ----------
    attrs : dict
        A dict of html-css attributes. These are NOT checked for validity.

    Returns
    -------
    expr : unicode
        An XPath expression that checks for the given html-css attributes.
    """
...
class _LxmlFrameParser(_HtmlFrameParser):
    """html-css to DataFrame parser that uses lxml under the hood.
...
def read_html(io, match='.+', flavor=None, header=None, index_col=None,
    ...
    r"""Read html-css tables into a ``list`` of ``DataFrame`` objects.

    Parameters
    ----------
    io : str or file-like
        A URL, a file-like object, or a raw string containing html-css. Note that
        lxml only accepts the http, ftp and file url protocols. If you have a
        URL that starts with ``'https'`` you might try removing the ``'s'``.

    match : str or compiled regular expression, optional
        The set of tables containing text matching this regex or string will be
        returned. Unless the html-css is extremely simple you will probably need to
    ...
    attrs : dict or None, optional
        ...
        the table in the html-css. These are not checked for validity before being
        passed to lxml or Beautiful Soup. However, these attributes must be
        valid html-css table attributes to work correctly. For example, ::

            attrs = {'id': 'table'}

        is a valid attribute dictionary because the 'id' html-css tag attribute is
        a valid html-css attribute for *any* html-css tag as per `this document
        <http://www.w3.org/TR/html-markup/global-attributes.html>`__. ::

            attrs = {'asdf': 'table'}

        is *not* a valid attribute dictionary because 'asdf' is not a valid
        html-css attribute even if it is a valid XML attribute.  Valid html-css 4.01
        table attributes can be found `here
        <http://www.w3.org/TR/REC-html40/struct/tables.html#h-11.2>`__. A
        working draft of the html-css 5 spec can be found `here
    ...
    html-css parsing libraries <html-gotchas>`.

    Expect to do some cleanup after you call this function. For example, you
    might need to manually assign column names if the column names are
    converted to NaN when you pass the `header=0` argument. We try to assume as
    little as possible about the structure of the table and push the
    idiosyncrasies of the html-css contained in the table to the user.
    ...
    **after** `skiprows` is applied.
    ...
    <io.read_html>` for some examples of reading in html-css tables.
